    ...
    while not jobs['quit']:
        ...
        for motor in (MOTOR0, MOTOR1):
            if jobs[motor]['id'] != doing[motor]['id']:
                ...
                target = 0
                if doing[motor]['operation'] == 'closing':
                    target = status[motor]['position'] + jobs[motor]['steps']
                    if target > TILT_STEPS:
                        target = TILT_STEPS
                elif doing[motor]['operation'] == 'opening':
                    target = status[motor]['position'] - jobs[motor]['steps']
                    if target < 0:
                        target = 0
                doing[motor]['target'] = target
...
def tilt(motor, steps, hs_dur=TILT_HS_DUR):
    ...
    if motor == MOTOR0 or motor == MOTOR1:
        ...
        if steps > 0:
            jobs[motor]['operation'] = 'closing'
            jobs[motor]['steps'] = steps
        elif steps < 0:
            jobs[motor]['operation'] = 'opening'
            jobs[motor]['steps'] = -steps
        else:
            raise Exception("Steps cannot be 0!")
